...
echo "Starting master tracker on :56051 (dashboard :18080)..."
...
for i in $(seq 1 "${NUM_NODES}"); do
    ...
    ./bin/node -id "${node_id}" -port "${port}" -master localhost:56051 > "logs/${node_id}.log" 2>&1 &
done

echo "Starting client UI on :18081..."
./bin/client -master localhost:56051 -port 18081 > logs/client.log 2>&1 &

echo
echo "Local smoke-test environment is ready."
echo "  Master dashboard: http://localhost:18080"
echo "  Client UI:        http://localhost:18081"
echo "  Logs:             ./logs/"
echo
